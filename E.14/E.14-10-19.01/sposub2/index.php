...
<body>
	...
	<div id="glownyL">
		...
		<?php
			$conn =new mysqli('localhost','root','','baza');
		 	$qr="SELECT zwierzeta.gatunek, odzywianie.rodzaj FROM zwierzeta INNER JOIN odzywianie ON odzywianie.id = zwierzeta.Odzywianie_id";
	...
	<div id="glownyS">
		<?php
			...
			{	
				$list = $_POST['list'];
		 		$qr="SELECT zwierzeta.id, zwierzeta.gatunek, zwierzeta.wystepowanie FROM zwierzeta INNER JOIN odzywianie ON odzywianie.id = zwierzeta.Odzywianie_id WHERE zwierzeta.Odzywianie_id = $list ";		 
			   	if ($list == 1)
				{

...
<body>
	...
	<div id="glownyL">
		...
		<?php
			$polonczenie =mysqli_connect('localhost','root','','baza');
		 	$query= mysqli_query($polonczenie,"SELECT `zwierzeta`.`gatunek`, `odzywianie`.`rodzaj` FROM `zwierzeta` INNER JOIN `odzywianie` ON `odzywianie`.`id` = `zwierzeta`.`Odzywianie_id`");
	...
	<div id="glownyS">
		<?php
			...
			{	
				$list = $_POST['list'];
		 		$zapytanie= mysqli_query($connect,"SELECT `zwierzeta`.`id`, `zwierzeta`.`gatunek`, `zwierzeta`.`wystepowanie` FROM `zwierzeta` INNER JOIN `odzywianie` ON `odzywianie`.`id` = `zwierzeta`.`Odzywianie_id` WHERE `zwierzeta`.`Odzywianie_id` = $list ");		 
			   	if ($list == 1)
				{
